export let READ_MASTER_DATA_ENTRY_ID_BY_NAME = "SELECT mde.MASTER_DATA_ENTRY_ID FROM MASTER_DATA_ENTRY mde WHERE LOWER(mde.NAME) = '{name}'";
export let READ_CATEGORY_ID_BY_NAME = "SELECT cc.CONTENT_CATEGORY_ID FROM CONTENT_CATEGORY cc WHERE LOWER(cc.NAME) = '{name}'";
export let READ_CONTENT_ID_BY_PATH = "select regexp_substr('{path}', '[^/]+$', 1, 1) FROM CONTENT_INFORMATION ci";


export let WRITE_MASTER_DATA_ENTRY =
...
export let WRITE_CONTENT_CATEGORY =
        "INSERT INTO CONTENT_CATEGORY ( category_type, name, name_local, is_published, description, thumb_image_path, "
                ...
                + "VALUES ( ({categoryType}), '{name}', '{nameLocal}', {isPublished}, '{description}', '{thumbImagePath}', {vieOrder}, 1, CURRENT_TIMESTAMP, 1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)";

export let WRITE_CONTENT_INFORMATION =
...
export let WRITE_CONTENT_INFORMATION_TAGS =
